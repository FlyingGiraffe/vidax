...
calls the real `examples/generate_hunyuan_video_1_5.py`'s `main(args)` (no
separate reimplementation of the generation loop -- see `benchmarks/
common.py`'s module docstring).

Usage:
    python benchmarks/run_hunyuan_video_1_5.py --resolution 480p --task t2v
    VIDAX_CHECKPOINT_DIR=/mnt/disks/tpu_ssd/checkpoints python benchmarks/run_hunyuan_video_1_5.py --resolution 720p --task i2v
"""
import argparse
import os
import sys
...
import generate_hunyuan_video_1_5  # noqa: E402
...
# (height, width) per --resolution -- matches examples/generate_hunyuan_video_1_5.py's
...
# --vae_tile_latent_size 8 (see docs/lessons/hunyuan_video_1_5_debugging.md's
...
def build_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    ns = argparse.Namespace(
        ...
        # left None so generate_hunyuan_video_1_5.main derives (height,
        ...
        output_path=common.output_path("hunyuan_video", "_1_5", args.resolution, args.task),
    )
    if args.task == "i2v":
...
def main():
    ...
    parser.add_argument(
        ...
        help="Overrides the default VAE spatial-tile size (8) -- see "
             "docs/lessons/hunyuan_video_1_5_debugging.md's 'VAE tile size vs. TP memory headroom'.")
    args = parser.parse_args()

    run_args = build_args(args)
    common.run_benchmark(
        model="hunyuan_video", version="_1_5", size=args.resolution, task=args.task,
        main_fn=generate_hunyuan_video_1_5.main, args=run_args, num_runs=args.num_runs)
